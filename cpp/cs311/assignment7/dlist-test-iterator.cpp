...
// Test file for class DList - iterator version
                      ...
                      const std::string & s)
{
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        oss << (i+1);
        t.test(*cit == *valIter, s + " - iterate, item " + oss.str());
    ...
    for (int k = size-1; k >= 0; --k)
    {
        std::ostringstream oss;
        oss << (k+1);
        t.test(*cit == *valIter, s + " - backward iterate, item " + oss.str());
        cit2 = cit;
        --cit;
        --valIter;
    }
...
// ************************************************************************
// Iterator Category Checking
// ************************************************************************

// tagIsBidirectional
// This pair of functions returns true if the type of the object passed
// is std::bidirectional_iterator_tag, false otherwise.
// Requirements on types: None.
// Pre: None.
// Post:
//     Return is true for object of type std::bidirectional_iterator_tag,
//     false otherwise.
// Does not throw (No-Throw Guarantee)
bool tagIsBidirectional_internal(std::bidirectional_iterator_tag dummy)
{ return true; }

template <typename Other>
bool tagIsBidirectional_internal(Other dummy)
{ return false; }

// tagIsBidirectional
// Given an iterator, returns true if its category is bidirectional,
// false for any other category, including random access.
// Requirements on types:
//     Iter must define iterator traits.
// Pre: None.
// Post:
//     Return is true iff category of iter is bidirectional.
// Does not throw (No-Throw Guarantee)
template <typename Iter>
bool tagIsBidirectional(Iter it)
{
    typedef typename std::iterator_traits<Iter>::iterator_category Category;
    return tagIsBidirectional_internal(Category());
}
...
void test_class_DList_insert_begin_end_iterators(Tester & t)
{
    ...
    t.test(*cit == 1, "Insert 1, const_iterator to const obj - item");
    ...
    t.test(*cit == 1, "Insert 1, const_iterator to non-const obj - item");
    ...
    t.test(*it == 1, "Insert 1, iterator - item");
...
// test_class_DList_iterator_ops
// Test suite for class DList, iterator operations
// Pre: None.
// Post:
//     Pass/fail status of tests have been registered with t.
//     Appropriate have been messages printed to cout.
// Does not throw (No-Throw Guarantee)
void test_class_DList_iterator_ops(Tester & t)
{
    std::cout << "Test Suite: class DList - iterator operations" << std::endl;

    DList d1;
    DList::iterator it1;
    DList::const_iterator cit1;

    d1.insert(d1.end(), 10);
    d1.insert(d1.end(), 2);
    d1.insert(d1.end(), 3);

    // DEREFERENCE, INC, DEC TESTS

    it1 = d1.begin();

    // Check iterator dereference
    t.test(*it1 == 10, "iterator dereference read");
    *it1 = 1;
    t.test(*it1 == 1, "iterator dereference write");

    // Check iterator copy ctor
    DList::iterator it2(it1);
    t.test(*it2 == 1, "iterator copy ctor");

    // Check iterator pre-inc
    it2 = (++it1);
    t.test(*it1 == 2, "iterator pre-inc, effect");
    t.test(*it2 == 2, "iterator pre-inc, return value");

    // Check iterator post-inc
    it2 = (it1++);
    t.test(*it1 == 3, "iterator post-inc, effect");
    t.test(*it2 == 2, "iterator post-inc, return value");

    // Check iterator pre-dec
    it2 = (--it1);
    t.test(*it1 == 2, "iterator pre-dec, effect");
    t.test(*it2 == 2, "iterator pre-dec, return value");

    // Check iterator post-dec
    it2 = (it1--);
    t.test(*it1 == 1, "iterator post-dec, effect");
    t.test(*it2 == 2, "iterator post-dec, return value");

    // const_iterator tests
    cit1 = d1.begin();

    // Check const_iterator dereference
    t.test(*it1 == 1, "const_iterator dereference read");

    // Check iterator copy ctor
    DList::const_iterator cit2(cit1);
    t.test(*cit2 == 1, "const_iterator copy ctor");

    // Check const_iterator pre-inc
    cit2 = (++cit1);
    t.test(*cit1 == 2, "const_iterator pre-inc, effect");
    t.test(*cit2 == 2, "const_iterator pre-inc, return value");

    // Check const_iterator post-inc
    cit2 = (cit1++);
    t.test(*cit1 == 3, "const_iterator post-inc, effect");
    t.test(*cit2 == 2, "const_iterator post-inc, return value");

    // Check const_iterator pre-dec
    cit2 = (--cit1);
    t.test(*cit1 == 2, "const_iterator pre-dec, effect");
    t.test(*cit2 == 2, "const_iterator pre-dec, return value");

    // Check const_iterator post-dec
    cit2 = (cit1--);
    t.test(*cit1 == 1, "const_iterator post-dec, effect");
    t.test(*cit2 == 2, "const_iterator post-dec, return value");

    // EQUALITY/INEQUALITY TESTS

    it1 = d1.begin();
    it2 = it1;
    DList::iterator it3 = it1;
    ++it3;
    cit1 = d1.begin();
    cit2 = cit1;
    DList::const_iterator cit3 = cit1;
    ++cit3;

    t.test(it1 == it2, "iterator == iterator #1");
    t.test(!(it1 == it3), "iterator == iterator #2");
    t.test(!(it1 != it2), "iterator != iterator #1");
    t.test(it1 != it3, "iterator != iterator #2");

    t.test(it1 == cit2, "iterator == const_iterator #1");
    t.test(!(it1 == cit3), "iterator == const_iterator #2");
    t.test(!(it1 != cit2), "iterator != const_iterator #1");
    t.test(it1 != cit3, "iterator != const_iterator #2");

    t.test(cit1 == it2, "const_iterator == iterator #1");
    t.test(!(cit1 == it3), "const_iterator == iterator #2");
    t.test(!(cit1 != it2), "const_iterator != iterator #1");
    t.test(cit1 != it3, "const_iterator != iterator #2");

    t.test(cit1 == cit2, "const_iterator == const_iterator #1");
    t.test(!(cit1 == cit3), "const_iterator == const_iterator #2");
    t.test(!(cit1 != cit2), "const_iterator != const_iterator #1");
    t.test(cit1 != cit3, "const_iterator != const_iterator #2");

    // COPY ITERATOR TO CONST_ITERATOR TESTS

    it1 = d1.begin();
    DList::const_iterator cit4(it1);
    t.test(cit4 == d1.begin(), "const_iterator copy constructed from iterator");
    DList::const_iterator cit5;
    cit5 = it1;
    t.test(cit4 == d1.begin(), "const_iterator copy assigned from iterator");

    // ITERATOR TAG TESTS

    t.test(tagIsBidirectional(it1), "iterator - bidirectional iterator tag");
    t.test(tagIsBidirectional(cit1), "const_iterator - bidirectional iterator tag");
}
...
void test_class_DList(Tester & t)
{
    ...
    test_class_DList_iterator_ops(t);
}
